...
import { CancelWithdrawRequestDTO, CreateWithdrawRequestDTO } from "./dto/createWithdrawRequest.dto";
...
export class WithdrawController{
    ...
    @Post("cancel")
    async cancelWithdrawRequest(@Res()res:Response , req:AuthenticatedRequest , 
        @Body() body: CancelWithdrawRequestDTO,
        ){
            try {
                const withdrawRequestID =body.reqID
                const userID = req.user.userID
                const withdrawRequest = await this.withdrawService.cancelWithdrawRequest(withdrawRequestID, userID)
                if(withdrawRequest){
                    res.status(HttpStatus.OK).json({msg:"Withdraw request updated successfully ", data:withdrawRequest, success:true})
                }else{
                    res.status(HttpStatus.BAD_REQUEST).json({msg:"Withdraw request not updated successfiully" , data:null, success:false})
                }
